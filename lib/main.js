...
exports.getScssCode = function (classIdMap, mapFilter) {
    var scss = '';
    for (var p in classIdMap) {
        if (classIdMap.hasOwnProperty(p) && !invalidKey(p)) {
            if (mapFilter) {
                if (mapFilter(p, getTypeFromClassIdMapAndKey(classIdMap, p))) {
                    scss += '$' + p + ': ' + classIdMap[p] + ';\n';
                }
            } else {
                scss += '$' + p + ': ' + classIdMap[p] + ';\n';
...
exports.getDevScssCode = function (classIdMap) {
    var scss = '';
    for (var p in classIdMap) {
        if (classIdMap.hasOwnProperty(p) && !invalidKey(p)) {
            scss += '$' + p + ': ' + p + ';\n';
